...
def exact(fun, x, d, eps=1e-2, a_min=0.01, a_max=10):
...
def inexact(fun, x, d, a_min=0.01, a_max=10,
            rho=1e-4, sigma=0.9, fx=None, gx=None):
    """Inexact line search: strong Wolfe condition
    ...
    while True:
        phi_a1 = phi(a1)
        if (phi_a1 > phi_0 + rho * a1 * phip_0) \
            or (a0 > 0 and phi_a1 >= phi_a0):
            zoom, v = _zoom(a0, a1, phi, phip,
        ...
        if abs(phip_a1) <= -sigma * phip_0:
            return a1, neval
        if phip_a1 >= 0:
            zoom, v = _zoom(a1, a0, phi, phip,
                            phi_0, phip_0, phi_a1, rho, sigma)
            return zoom, neval + v

        # Choose an alpha from (a1, a_max)
        a0 = a1
        anew = _cubicInterplt(a1, phi_a1, phip_a1, a0, phi_a0, a_max, phi_max)
        anew_lo = a1 + 0.1 * (a_max - a1)
        anew_hi = a_max - 0.1 * (a_max - a1)
        if (anew is not None) and (anew > anew_lo) and (anew < anew_hi):
            a1 = anew
        else:
            anew = _quadInterplt(a1, phi_a1, phip_a1, a_max, phi_max)
            if (anew is not None) and (anew > anew_lo) and (anew < anew_hi):
                a1 = anew
            else:
                a1 = (a1 + a_max) / 2
